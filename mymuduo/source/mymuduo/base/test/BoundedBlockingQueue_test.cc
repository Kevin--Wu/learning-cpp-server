 ...
 public:
      ...
      latch_(numThreads)
  {
    ...
    for (int i = 0; i < numThreads; ++i)
    {
      ...
      threads_.emplace_back(new mymuduo::Thread(
            std::bind(&Test::threadFunc, this), mymuduo::string(name)));
    }
  ...
  void run(int times)
  {
    ...
    for (int i = 0; i < times; ++i)
    {
      ...
      queue_.put(buf);
      printf("tid=%d, put data = %s, size = %zd\n", mymuduo::CurrentThread::tid(), buf, queue_.size());
    }
  }
 ...
 private:
  ...
  void threadFunc()
  {
    ...
    while (running)
    {
      ...
      printf("tid=%d, get data = %s, size = %zd\n", mymuduo::CurrentThread::tid(), d.c_str(), queue_.size());
      running = (d != "stop");
    }
  ...
  mymuduo::BoundedBlockingQueue<std::string> queue_;
...
void testMove()
{
  ...
  mymuduo::BoundedBlockingQueue<std::unique_ptr<int>> queue(10);
